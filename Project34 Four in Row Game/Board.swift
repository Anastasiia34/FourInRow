...
//
//  Created by Анастасия Стрекалова on 01.04.2020.
...
class Board: NSObject, GKGameModel {
    ...
    func isWin(for player: GKGameModelPlayer) -> Bool {
        ...
        for row in 0 ..< Board.height {
            for column in 0 ..< Board.width {
                if squaresMatch(initialChip: chip, row: row, column: column, moveX: 1, moveY: 0) {
                    return true
                } else if squaresMatch(initialChip: chip, row: row, column: column, moveX: 0, moveY: 1) {
                    return true
                } else if squaresMatch(initialChip: chip, row: row, column: column, moveX: 1, moveY: 1) {
                    return true
                } else if squaresMatch(initialChip: chip, row: row, column: column, moveX: 1, moveY: -1) {
                    return true
                }
